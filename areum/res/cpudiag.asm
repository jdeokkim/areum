...
CPU:	LXI	SP,STACK	;SET THE STACK POINTER
	ANI	0	;INITIALIZE A REG. AND CLEAR ALL FLAGS <- 0x01ae
	JZ	J010	;TEST "JZ"
	CALL	CPUER
...
J050:	CALL	CPUER
J060:	ADI	6	;A=6,C=0,P=1,S=0,Z=0 <- 0x01da
	JNZ	J070	;TEST "JNZ"
	CALL	CPUER
J070:	JC	J080	;TEST "JC"
	...
	JP	J090	;TEST "JP"
J080:	CALL	CPUER
J090:	ADI	070H	;A=76H,C=0,P=0,S=0,Z=0 <- 0x01ee
	JPO	J100	;TEST "JPO"
	CALL	CPUER
J100:	JM	J110	;TEST "JM"
	...
	JNC	J120	;TEST "JNC"
J110:	CALL	CPUER
J120:	ADI	081H	;A=F7H,C=0,P=0,S=1,Z=0 <- 0x0202 (ok)
	JM	J130	;TEST "JM"
	CALL	CPUER
J130:	JZ	J140	;TEST "JZ"
	...
	JPO	J150	;TEST "JPO"
J140:	CALL	CPUER
J150:	ADI	0FEH	;A=F5H,C=1,P=1,S=1,Z=0 <- 0x0216
	JC	J160	;TEST "JC"
	CALL	CPUER
...
;
AIMM:	CPI	0	;A=F5H,C=0,Z=0 <- 0x022a
	JC	CPIE	;TEST "CPI" FOR RE-SET CARRY
	JZ	CPIE	;TEST "CPI" FOR RE-SET ZERO
	CPI	0F5H	;A=F5H,C=0,Z=1 <- 0x0232
	JC	CPIE	;TEST "CPI" FOR RE-SET CARRY ("ADI")
	JNZ	CPIE	;TEST "CPI" FOR RE-SET ZERO
	CPI	0FFH	;A=F5H,C=1,Z=0 <- 0x023a
	JZ	CPIE	;TEST "CPI" FOR RE-SET ZERO
	JC	ACII	;TEST "CPI" FOR SET CARRY
CPIE:	CALL	CPUER
ACII:	ACI	00AH	;A=F5H+0AH+CARRY(1)=0,C=1 <- 0x0245
	ACI	00AH	;A=0+0AH+CARRY(0)=0BH,C=0 <- 0x0247
	CPI	00BH
	JZ	SUII	;TEST "ACI"
	CALL	CPUER
SUII:	SUI	00CH	;A=FFH,C=0 <- 0x0251
	SUI	00FH	;A=F0H,C=1 <- 0x0253
	CPI	0F0H
	JZ	SBII	;TEST "SUI"
	CALL	CPUER
SBII:	SBI	0F1H	;A=F0H-0F1H-CARRY(0)=FFH,C=1 <- 0x025d
	SBI	00EH	;A=FFH-OEH-CARRY(1)=F0H,C=0 <- 0x025f
	CPI	0F0H
	JZ	ANII	;TEST "SBI"
	CALL	CPUER
ANII:	ANI	055H	;A=F0H<AND>55H=50H,C=0,P=1,S=0,Z=0 <- 0x0269
	CPI	050H
	JZ	ORII	;TEST "ANI"
	CALL	CPUER
ORII:	ORI	03AH	;A=50H<OR>3AH=7AH,C=0,P=0,S=0,Z=0 <- 0x0273
	CPI	07AH
	JZ	XRII	;TEST "ORI"
	CALL	CPUER
XRII:	XRI	00FH	;A=7AH<XOR>0FH=75H,C=0,P=0,S=0,Z=0 <- 0x027d
	CPI	075H
	JZ	C010	;TEST "XRI"
	CALL	CPUER
;
...
;
C010:	ANI	000H	;A=0,C=0,P=1,S=0,Z=1 <- 0x0287
	CC	CPUER	;TEST "CC"
	CPO	CPUER	;TEST "CPO"
	CM	CPUER	;TEST "CM"
	CNZ	CPUER	;TEST "CNZ"
	CPI	000H
	JZ	C020	;A=0,C=0,P=0,S=0,Z=1 <- 0x0297
	CALL	CPUER
C020:	SUI	077H	;A=89H,C=1,P=0,S=1,Z=0 <- 0x029d
	CNC	CPUER	;TEST "CNC"
	CPE	CPUER	;TEST "CPE"
	CP	CPUER	;TEST "CP"
	CZ	CPUER	;TEST "CZ"
...
MOVI:	MVI	A,077H <- 0x031d
	...
	MOV	A,B
	CPI	077H
	CNZ	CPUER	;TEST "MOV" A,B,C,D,E,H,L,A <- 0x0359
;
;
;
...
;
	...
	ADD	A
	CPI	0F0H
	CNZ	CPUER	;TEST "ADD" B,C,D,E,H,L,A <- 0x0372
	SUB	B
	SUB	C
	SUB	D
	SUB	E
	...
	SUB	L
	CPI	078H
	CNZ	CPUER	;TEST "SUB" B,C,D,E,H,L <- 0x037d
	SUB	A
	CNZ	CPUER	;TEST "SUB" A <- 0x0381
	MVI	A,080H
	ADD	A
	MVI	B,001H
	MVI	C,002H
	...
	ADC	A
	CPI	037H
	CNZ	CPUER	;TEST "ADC" B,C,D,E,H,L,A <- 0x03aa
	MVI	A,080H
	ADD	A
	MVI	B,001H
	SBB	B
	...
	SBB	L
	CPI	0E0H
	CNZ	CPUER	;TEST "SBB" B,C,D,E,H,L <- 0x03c1
	MVI	A,080H
	ADD	A
	SBB	A
	CPI	0FFH
	CNZ	CPUER	;TEST "SBB" A <- 0x03ca
	MVI	A,0FFH
	MVI	B,0FEH
	MVI	C,0FCH
	MVI	D,0EFH
	...
	ANA	A
	CPI	024H
	CNZ	CPUER	;TEST "ANA" B,C,D,E,H,L,A <- 0x03e4
	XRA	A
	MVI	B,001H
	MVI	C,002H
	MVI	D,004H
	...
	ORA	A
	CPI	03FH
	CNZ	CPUER	;TEST "ORA" B,C,D,E,H,L,A <- 0x03fd
	MVI	A,000H
	MVI	H,08FH
	MVI	L,04FH
	XRA	B
	...
	XRA	L
	CPI	0CFH
	CNZ	CPUER	;TEST "XRA" B,C,D,E,H,L <- 0x040e
	XRA	A
	CNZ	CPUER	;TEST "XRA" A <- 0x0412
	MVI	B,044H
	MVI	C,045H
	MVI	D,046H
	MVI	E,047H
	...
	MVI	A,044H
	CMP	B
	CNZ	CPUER	;TEST "MOV" M,B AND B,M <- 0x0428
	MOV	M,D
	MVI	D,000H
	MOV	D,M
	MVI	A,046H
	CMP	D
	CNZ	CPUER	;TEST "MOV" M,D AND D,M <- 0x0432
	MOV	M,E
	MVI	E,000H
	MOV	E,M
	MVI	A,047H
	CMP	E
	CNZ	CPUER	;TEST "MOV" M,E AND E,M <- 0x043c
	MOV	M,H
	MVI	H,(TEMP0 / 0FFH)
	...
	MVI	A,(TEMP0 / 0FFH)
	CMP	H
	CNZ	CPUER	;TEST "MOV" M,H AND H,M <- 0x0448
	MOV	M,L
	MVI	H,(TEMP0 / 0FFH)
	MVI	L,(TEMP0 AND 0FFH)
	MOV	L,M
	...
	CMP	L
	CNZ	CPUER	;TEST "MOV" M,L AND L,M <- 0x0454
	MVI	H,(TEMP0 / 0FFH)
	MVI	L,(TEMP0 AND 0FFH)
	MVI	A,032H
	MOV	M,A
	CMP	M
	CNZ	CPUER	;TEST "MOV" M,A <- 0x045f
	ADD	M
	CPI	064H
	CNZ	CPUER	;TEST "ADD" M <- 0x0465
	XRA	A
	MOV	A,M
	CPI	032H
	CNZ	CPUER	;TEST "MOV" A,M <- 0x046c
	MVI	H,(TEMP0 / 0FFH)
	MVI	L,(TEMP0 AND 0FFH)
	MOV	A,M
	SUB	M
	CNZ	CPUER	;TEST "SUB" M <- 0x0475
	MVI	A,080H
	ADD	A
	ADC	M
	CPI	033H
	CNZ	CPUER	;TEST "ADC" M <- 0x047e
	MVI	A,080H
	ADD	A
	SBB	M
	CPI	0CDH
	CNZ	CPUER	;TEST "SBB" M <- 0x0487
	ANA	M
	CNZ	CPUER	;TEST "ANA" M <- 0x048b
	MVI	A,025H
	ORA	M
	CPI	037H
	CNZ	CPUER	;TEST "ORA" M <- 0x0493
	XRA	M
	CPI	005H
	CNZ	CPUER	;TEST "XRA" M <- 0x0499
	MVI	M,055H
	INR	M
	DCR	M
	ADD	M
	CPI	05AH
	CNZ	CPUER	;TEST "INR","DCR",AND "MVI" M <- 0x04a3
	LXI	B,12FFH
	LXI	D,12FFH
	LXI	H,12FFH
	INX	B
	...
	MVI	A,013H
	CMP	B
	CNZ	CPUER	;TEST "LXI" AND "INX" B <- 0x04b5
	CMP	D
	CNZ	CPUER	;TEST "LXI" AND "INX" D <- 0x04b8
	CMP	H
	CNZ	CPUER	;TEST "LXI" AND "INX" H <- 0x04bd
	MVI	A,000H
	CMP	C
	...
	CMP	L
	CNZ	CPUER	;TEST "LXI" AND "INX" H <- 0x04cb
	DCX	B
	DCX	D
	DCX	H
	MVI	A,012H
	CMP	B
	CNZ	CPUER	;TEST "DCX" B <- 0x04d4
	CMP	D
	...
	LDA	TEMP0
	CPI	0FFH
	CNZ	CPUER	;TEST "LDA" AND "STA" <- 0x04f6
	LHLD	TEMPP
	SHLD	TEMP0
	LDA	TEMPP
	MOV	B,A
	LDA	TEMP0
	CMP	B
	CNZ	CPUER	;TEST "LHLD" AND "SHLD" <- 0x0507
	LDA	TEMPP+1
	MOV	B,A
	LDA	TEMP0+1
	CMP	B
	CNZ	CPUER	;TEST "LHLD" AND "SHLD" <- 0x0512
	MVI	A,0AAH
	STA	TEMP0
	MOV	B,H
	MOV	C,L
	...
	LDAX	D
	CPI	077H
	CNZ	CPUER	;TEST "LDAX" D AND "XCHG" <- 0x053d
	XRA	A
	ADD	H
	ADD	L
	CNZ	CPUER	;TEST "XCHG" <- 0x0543
	MVI	A,0CCH
	STAX	D
	LDA	TEMP0
	CPI	0CCH
	...
	CPI	055H
	CNZ	CPUER	;TEST "CMA"
	ORA	A	;RE-SET AUXILIARY CARRY <- 0x059c
	DAA
	CPI	055H
	CNZ	CPUER	;TEST "DAA" <- 0x05a0
	MVI	A,088H
	ADD	A
	DAA
	CPI	076H
	CNZ	CPUER	;TEST "DAA" <- 0x05a9
	XRA	A
	MVI	A,0AAH
	DAA
	CNC	CPUER	;TEST "DAA" <- 0x05b0
	CPI	010H
	CNZ	CPUER	;TEST "DAA" <- 0x05b5
	XRA	A
	MVI	A,09AH
	DAA
	CNC	CPUER	;TEST "DAA" <- 0x05bc
	CNZ	CPUER	;TEST "DAA"
	STC
	MVI	A,042H
	RLC
	CC	CPUER	;TEST "RLC" FOR RE-SET CARRY <- 0x05c6
	RLC
	...
	RAL
	RAL
	CNC	CPUER	;TEST "RAL" FOR SET CARRY <- 0x05de
	CPI	008H
	CNZ	CPUER	;TEST "RAL" FOR ROTATION
	RAR
	RAR
	CC	CPUER	;TEST "RAR" FOR RE-SET CARRY <- 0x05e8
	CPI	002H
	...
	POP	D
	POP	B
	CC	CPUER	;TEST "PUSH PSW" AND "POP PSW" <- 0x060f
	...
	CMP	L
	CNZ	CPUER	;TEST "PUSH H" AND "POP H" <- 0x0638
	LXI	H,00000H
	DAD	SP
	SHLD	SAVSTK	;SAVE THE "OLD" STACK-POINTER! <- 0x063f
	LXI	SP,TEMP4
	DCX	SP
	DCX	SP
	INX	SP
	...
	CMA
	CMP	C
	CNZ	CPUER	;TEST "LXI","DAD","INX", AND "DCX" SP <- 0x0659
	LXI	H,TEMP4
	SPHL
	LXI	H,07733H
	DCX	SP
	...
	LDA	TEMP3
	CPI	077H
	CNZ	CPUER	;TEST "SPHL" AND "XTHL" <- 0x066b
	LDA	TEMP2
	CPI	033H
	CNZ	CPUER	;TEST "SPHL" AND "XTHL" <- 0x0673
	MVI	A,055H
	CMP	L
	CNZ	CPUER	;TEST "SPHL" AND "XTHL" <- 0x0679
	CMA
	CMP	H
	CNZ	CPUER	;TEST "SPHL" AND "XTHL" <- 0x067e
